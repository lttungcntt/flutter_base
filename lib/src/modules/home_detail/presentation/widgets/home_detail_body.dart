...
class HomeDetailBody extends ConsumerWidget {
      ...
      (_, nov) {
        if (nov.status.isError) {
          context.showError(nov.error?.message ?? 'Error', onPressed: () {
            ref.read(homeDetailProvider.bloc).get();
          });
        }
      },
    );
    final state = ref.read(homeDetailProvider.select((state) => state));
    return BackgroundContainer(
      height: context.mediaQuery.size.height,
      child: RefreshIndicator(
        onRefresh: () async {
          await ref.read(homeDetailProvider.bloc).get();
        },
